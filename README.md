...
| **18650 lithium battery** | check price | Fits in the holder on the Waveshare board. Use a good-quality protected cell. |
| **Small speaker** with an MX1.25 2-pin plug | a few $ | Plugs into the Waveshare board's speaker header. Without it there is no vario sound. |
...
2. Upload the file [`PCB files/Gerber Files V2/ESP32 vario-B_PCB_.zip`](PCB%20files/Gerber%20Files%20V2/). It contains all the Gerber and drill files in one zip.
3. Leave the options at their defaults unless you have a preference (it is a simple two-layer board), and place the order.

The original design files (KiCad) are in [`PCB files/ESP32 vario_PCB_layout/`](PCB%20files/ESP32%20vario_PCB_layout/) if you want to modify the board.
...
| Button (short) | `button_Short.stl` | 6 m |
| Button (long) | `button_Long.stl` | 9 m |
...
![The PCB with the LoRa module fitted](Photos/20260916_134729.jpg)
...
![Assembled unit](Photos/20260919_102321.jpg)
...
**How it works:** the temperature and humidity sensor on the Waveshare board gives the *dew point*. The gap between the air temperature and the dew point tells you how far the air has to rise before it cools enough to form cloud (roughly 125 m of height per 1°C of gap). The barometer's pressure reading is used to convert that into a height. It updates every 10 seconds and is smoothed so the number doesn't jump around.

**Good to know:**
- It is a **rough guide**, best for daytime cumulus-type conditions where the air is well mixed. It is not meaningful in stable or layered conditions, and it becomes less accurate the higher the base is.
- Don't rely on it as your only source of cloud information.
- The sensor is inside the case, so it lags behind the real air when conditions change quickly.

### Calibrate it (recommended)

The temperature sensor sits next to the processor and battery, so it reads **warmer than the outside air**. This makes the cloud base read **too high**, by about **125 m for every 1°C** of error. You can fix this with one number:
...
The estimate needs no extra parts or libraries. It uses the sensors already on the board and is calculated in the `CloudBase.h` file in the code folder.
...
- To make your own from a New Zealand LINZ elevation download (GeoTIFF), use the Python script [`dem_to_agldem.py`](paraglide_vario_ESP32/dem_to_agldem.py). The script's comments explain how it works.

### Engine meter (paramotor)
The [`Engine_meter/`](Engine_meter/) folder contains a separate project (nRF52840-based) that measures RPM and cylinder head temperature and sends them to the flight computer over Bluetooth. It is not needed for paragliding.
